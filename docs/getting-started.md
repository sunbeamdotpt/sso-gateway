...
export KRATOS_DEFAULT_SCHEMA_ID="default"
...
`KRATOS_DEFAULT_SCHEMA_ID="default"` matches the local Compose setup, which mounts
`deploy/kratos-identity.schema.json` as Kratos's base identity schema. That schema
requires only `traits.email`; the gateway owns and validates all tenant traits.
